...
//				$this->assignStuGrade("1500502118");
//				$this->display();
//				return;
//			}
...
//            $this->assign("getAT",$getTokenRes);

            //如果获取成功
            //dump($getTokenRes);
            if ($getTokenRes['status']) {

                $usr = M('stu_yb_id')->find($this->yb_userid);
...
//        }
        $this->assign("this_yb_userid",$this->yb_userid);
        $this->display();
        //exit;
    }
    ...
    private function getAccessToken($code) {
        //如果能够从缓存中获取
        if (isset($_SESSION['access_token']) && isset($_SESSION['yb_userid'])) {
            $this->yb_userid = $_SESSION['yb_userid'];
            return array('status'=>true,'assessToken'=>$_SESSION['access_token']);
        }
        ...
        $this->assign("getAT",$res);

        if ($res['access_token']) {
            //dump($res);//e4426222cabb32e29cefe350e9c95faf40c2250c
            //dump(date('Y-m-d H:i:s',$res['expires']));
            //进行缓存操作
            $this->yb_userid = $_SESSION['yb_userid'] = $res['userid'];
            $this->access_token =$_SESSION['access_token']= $res['access_token'];
            ...
            //msgEN 英文提示
            //dump($res);//e4426222cabb32e29cefe350e9c95faf40c2250c
            return array('status'=>false,'info'=>$res['info']['msgCN'],'code'=>$res['info']['code']);
        }
    }
    ...
    private function assignStuGrade($stu_id) {
        ...
        $this->assign("this_stu_id",$stu_id);
    }
